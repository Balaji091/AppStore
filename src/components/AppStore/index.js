...
const tabsList = [
  ...
  class AppStore extends Component{
    state={searchInput:"",trackTab:"SOCIAL"}
    setInput=(event)=>{
        this.setState({searchInput:event.target.value})
    }
    setCategory=(tabId)=>{
        this.setState({trackTab:tabId})

    }
    render(){
        const{searchInput,trackTab}=this.state
        const filterData = appsList.filter(item => 
            item.category===trackTab);
        const resultAppList = filterData.filter(item => 
        ...
        return(
            <div className='app-container'>
                 <h1>App Store</h1>
                 <div className='input'>
                    <input type='search' placeholder='search' value={searchInput} onChange={this.setInput}/>
                ...
                <div className='tab-container'>
                    <ul>
                        {
                            tabsList.map(item=><TabItem tabDetails={item} key={item.tabId} setCategory={this.setCategory} trackTab={trackTab}/>)
                        }
                    </ul>
                </div>
                <div className='app-list'>
                    <ul>
                        ...
                        { resultAppList.length>0?(resultAppList.map(item=>(<AppItem TabDetails={item} key={item.appId}/>))):<p className='no-results'> no apps found</p>}
                    </ul>
                </div>
